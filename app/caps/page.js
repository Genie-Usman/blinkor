...
const Caps = async () => {
  await connectDB();
  const products = await Products.find({ category: "caps" }).lean();
    ...
    if (caps[item.title]) {
      for (const variant of item.variants) {
        if (variant.availableQuantity > 0) {
          if (!caps[item.title].colors.includes(variant.color)) {
            caps[item.title].colors.push(variant.color);
          }
          if (!caps[item.title].sizes.includes(variant.size)) {
            caps[item.title].sizes.push(variant.size);
          }
        }
      }
    } else {
