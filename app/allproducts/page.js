...
const AllProducts = async () => {
    await connectDB();
    const products = await Products.find({ category: { $in: ["minion", "tshirts", "hoodies", "caps", "mugs"] } }).lean();
    ...
    for (const item of products) {
        ...
        if (allProducts[item.title]) {
            for (const variant of item.variants) {
                if (variant.availableQuantity > 0) {
                    if (!allProducts[item.title].colors.includes(variant.color)) {
                        allProducts[item.title].colors.push(variant.color);
                    }
                    if (!allProducts[item.title].sizes.includes(variant.size)) {
                        allProducts[item.title].sizes.push(variant.size);
                    }
                }
            }
        } else {
            ...
            <div className="container px-9 py-20 mx-auto">
                <h2 className="text-4xl font-extrabold text-gray-900 text-center tracking-wide uppercase">
                    <span className="text-[#C85C3D] font-bold text-4xl tracking-wide uppercase">
                        SHOP EVERYTHING: <span className="text-[#F4A261]">NO LIMITS, JUST STYLE!</span>
                    </span>
